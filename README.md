...
- s3 버킷의 original폴더에 이미지가 업로드됨

## v1.0
- aws lambda 사용하기
- s3에 올린 이미지를 리사이징한 후 줄어든 이미지를 다시 s3에 저장
- 이미지 리사이징은 cpu를 많이 사용하는 작업이기 때문에 lambda로 분리
- aws-upload 프로젝트를 통해 lambda를 사용
    - post.js : 이미지를 업로드할 때 기존 주소에서 original폴더 부분을 thumb 폴더로 교체 하도록 수정
    - main.pug : 이미지 미리보기 시에는 원본 이미지를 보여주고, 이미지 저장 후에는 리사이징된 이미지를 보여주도록 수정
        - 이미지 업로드와 이미지 리사이징 간의 시간차가 있으므로 이미지가 보이지 않을 수도 있는데, 그 문제를 해결하기 위와 같이 함
    - 람다 함수의 런타임을 12로 하니 `Error: write EPIPE` 에러가 나며 thumb에 저장이 되지않았음 -> 런타임을 10으로 바꾸니 thumb에 저장됨
